...
if (PRODUCTION) {
	...
	const serv = https.createServer(ssl, function (req, res) {});
	serv.listen(8085);
...
function loadmaps () {
	fs.readFile(path.join(__dirname, 'maps.txt'), 'utf-8', function (err, data) {
		if (err) {
			console.log(err);
		} else {
			...
			for (var p in playerListSecure) {
				if (p === clients[c]) {
					...
					newListToSend[clients[c]] = [
						...
						curPlayer.height,
						curPlayer.health,
						curPlayer.isJihad ? 1 : 0, // 1 and 0 take less bytes than "true" or "false"
						curPlayer.isInvuln ? 1 : 0, // 1 and 0 take less bytes than "true" or "false"
						[ // Compress bonuses
		...
		for (var b = 0; b < bullets.length; b++) {
			...
			if (Math.abs(centerOfCurPlayer.x - centerOfBullet.x) <= canvas.width + (0.5 * weaponsList[bullets[b].weapon].bulletSize)) {
				...
				if (Math.abs(centerOfCurPlayer.y - centerOfBullet.y) <= canvas.height + (0.5 * weaponsList[bullets[b].weapon].bulletSize)) {
					...
					newListToSend.push([
						Math.round(bullets[b].x),
						Math.round(bullets[b].y),
						weaponsList[bullets[b].weapon].id
					]);
				}
			}
		}
		...
		for (var e = 0; e < explosions.length; e++) {
			if (Math.abs(centerOfCurPlayer.x - explosions[e].x) <= canvas.width + (0.5 * curPlayer.width)) {
				if (Math.abs(centerOfCurPlayer.y - explosions[e].y) <= canvas.height + (0.5 * curPlayer.height)) {
					newListToSend.push([
						Math.round(explosions[e].x),
						Math.round(explosions[e].y),
						Math.round(explosions[e].curRadius)
					]);
				}
			}
		}
			...
			if (Math.abs(centerOfExtra.y - centerOfPlayer.y) <= 60 + player.height) {
				if (maps[currentMap].extrMap[maps[currentMap].activeExtras[e]].extra >= 10 && maps[currentMap].extrMap[maps[currentMap].activeExtras[e]].extra <= 17) {
					...
					if (maps[currentMap].extrMap[maps[currentMap].activeExtras[e]].extra <= 13) {
						newExtra = maps[currentMap].extrMap[maps[currentMap].activeExtras[e]].extra + 4;
					} else {
						newExtra = maps[currentMap].extrMap[maps[currentMap].activeExtras[e]].extra - 4;
					}
...
function testCollisionBlocks (newPlayer, oldPlayer) {
	...
	for (var b = 0; b < pColBlocks.length; b++) {
		if (testCollision(newPlayer, pColBlocks[b])) {
			const newDistance = distanceFromBlock(oldPlayer, pColBlocks[b]);
	...
	for (var e in extraColBlocks) {
		for (var ec in extraColBlocks[e]) {
			if (testCollision(newPlayer, extraColBlocks[e][ec])) {
				const newDistance = distanceFromBlock(oldPlayer, extraColBlocks[e][ec]);
				if (newDistance.x < bestDistance.x) {
					bestDistance.x = newDistance.x;
...
function update () {
	...
	for (let bt = 0; bt < bullets.length; bt++) {
		// Bullet movements
		...
		for (let b = 0; b < bColBlocks.length; b++) {
		...
		for (let e in extraColBlocks) {
			for (let ec in extraColBlocks[e]) {
				if (bullets[bt].active) {
					if (testBulletCollision({
						x: bullets[bt].x,
						y: bullets[bt].y,
		...
		for (let pl in playerListSecure) {
	...
	for (let p in playerListSecure) {
		...
		// Handle the bonuses
		for (let b in curPlayer.bonuses) {
...
io.on('connection', function (socket) {
	...
	socket.on('new-mouse-pos', function (mpos) {
		if (playerListSecure[userId]) {
			playerListSecure[userId].inputs.mousePos = {
				x: mpos[0],
				y: mpos[1]
			};
		}
